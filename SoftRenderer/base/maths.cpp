...
{
    Vector3f n = vector_normalize(Vector3f(vx, vy, vz));
...
{
    Vector3f zaxis = vector_normalize(eye - target);
    Vector3f xaxis = vector_normalize(vector_cross(up, zaxis));
    Vector3f yaxis = vector_cross(zaxis, xaxis);
    Matrix4x4 m;
    m.SetRow(0, Vector4f(xaxis.x, xaxis.y, xaxis.z, -vector_dot(eye, xaxis)));
    m.SetRow(1, Vector4f(yaxis.x, yaxis.y, yaxis.z, -vector_dot(eye, yaxis)));
    m.SetRow(2, Vector4f(zaxis.x, zaxis.y, zaxis.z, -vector_dot(eye, zaxis)));
